  ...
  const addToCart = async () => {
    ...
    if (isLoginUser) {
      ...
      if (cartData) {
        ...
        const duplicate = cartList.filter(
          (e: Item) => e.name === newItem.name && e.color === newItem.color
        );

        if (duplicate.length > 0) {
          const noSameProduct = cartList.filter(
            (e: Item) => e.name !== newItem.name || e.color !== newItem.color
          );
          const newData = [
            ...noSameProduct,
          ...
          await setDoc(doc(db, 'user', isLoginUser), { cart: newData }).then(() =>
            alert('장바구니에 담겼습니다.')
          );
        } else {
          const newData = [
        ...
        await setDoc(doc(db, 'user', isLoginUser), { cart: [{ ...newItem, id: 0 }] }).then(() =>
          alert('장바구니에 담겼습니다.')
        );
      }
    } else {
        ...
        <div className="productDetail__des__container__color">
          {product[0].color.map((e: string, i: number) => (
            <button
              ...
              onClick={() => setColor(e)}
            >
              <div className="circle" style={{ backgroundColor: `${e}` }}></div>
              <div>{e}</div>
            </button>
          ))}
        </div>
